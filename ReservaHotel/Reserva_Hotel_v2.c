...
// Função para comparar duas datas para determinar a ordem
int compareDates(const char* d1, const char* d2) {
    int dia1, mes1, ano1;
    int dia2, mes2, ano2;
    sscanf(d1, "%d/%d/%d", &dia1, &mes1, &ano1); // Dividir a data
    sscanf(d2, "%d/%d/%d", &dia2, &mes2, &ano2);

    if (ano1 < ano2) return -1; // -1 para anterior 
    if (ano1 > ano2) return 1; // 1 para posterior 
...
void splitList(No* source, No** frontRef, No** backRef) {
    No* fast; 
    No* slow;
    slow = source; // Aponta para o primeiro nó
    fast = source->prox; // Aponta para o segundo nó
    ...
    *frontRef = source; // A primeira sublista começa no início da lista original
    *backRef = slow->prox; // A segunda sublista começa no nó após o nó onde slow está posicionado.                             
    slow->prox = NULL; // O final da primeira sublista
    if (*backRef != NULL) {
        (*backRef)->ant = NULL; // O ponteiro ant do primeiro nó da segunda sublista é ajustado para NULL, mantendo a integridade da lista duplamente ligada.
    }
}
...
No* sortedMerge(No* a, No* b) {
    if (a == NULL) // Se a sublista a estiver vazia
        return b; // Retorna a sublista b
    if (b == NULL) // Se a sublista b estiver vazia
        return a; // Retorna a sublista a
    // Sem isso a função continuaria a chamar a si mesmo indefinidamente.

    No* result = NULL; // Armazenar o início da lista mesclada.

    //  Compara as datas das reservas nos nós a e b
    if (compareDates(a->reserva.data, b->reserva.data) <= 0) {
        result = a; // Se a tem data <= b selecionamos a como o nó atual
        result->prox = sortedMerge(a->prox, b); // Para o resultado da mesclagem
        if (result->prox != NULL)
            result->prox->ant = result; // Atualiza o ponteiro ant do nó seguinte para apontar para result
        result->ant = NULL;  // Garante que o nó atual não tenha um ponteiro ant incorreto
    }
    else {
        result = b; // Se b tem data <= a selecionamos a como o nó atual
...
void exibirReservas(No** listaRef) {
    ...
    lista = *listaRef; // Aponta para o primeiro nó da lista 
